    ...
    >
      <div className="flex items-start justify-between gap-3 mb-4">
        <div className="min-w-0">
          ...
          <p className="text-xs text-slate-500 mt-0.5">{scu.congestion_point?.operator}</p>
        </div>
        <div className={cn('badge shrink-0', congestionBg[severity])}>
          <span className={cn('w-1.5 h-1.5 rounded-full', congestionDot[severity])} />
          <span className={cn('text-[10px]', congestionColor[severity])}>{severity}</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <div>
          ...
          <p className="text-sm font-semibold text-white tabular">{scu.mwh ?? scu.mwh_amount} MWh</p>
