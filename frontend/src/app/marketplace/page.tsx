...
export default function MarketplacePage() {
  ...
  return (
    <AppShell>
      <div className="px-8 py-8 max-w-7xl mx-auto">
        ...
        <div className="card px-5 py-4 mb-6 flex flex-wrap gap-4 items-end">
          <div className="min-w-48">
            ...
            <select className="input" value={selectedPoint} onChange={(e) => setSelectedPoint(e.target.value)}>
          ...
          <div className="w-32">
            ...
            <input type="number" min="0" className="input" placeholder="0" value={minPrice} onChange={(e) => setMinPrice(e.target.value)} />
          </div>
          <div className="w-32">
            <label className="label block mb-2">Max price (€)</label>
            <input type="number" min="0" className="input" placeholder="Any" value={maxPrice} onChange={(e) => setMaxPrice(e.target.value)} />
          </div>
          {(selectedPoint || minPrice || maxPrice) && (
            <button className="btn-secondary text-sm" onClick={() => { setSelectedPoint(''); setMinPrice(''); setMaxPrice('') }}>
              Clear filters
            </button>
          )}
        </div>
            ...
            {[...Array(6)].map((_, i) => (
              <div key={i} className="card p-5 animate-pulse space-y-3">
                ...
                <div className="grid grid-cols-3 gap-3">
                  {[...Array(3)].map((_, j) => <div key={j} className="h-10 bg-surface-4 rounded" />)}
                </div>
              </div>
            ))}
          </div>
        ...
        {!loading && !error && scuList.length === 0 && (
          <div className="card px-5 py-16 text-center">
            ...
            <p className="text-sm text-slate-500 mt-1">Check back after the next matching cycle.</p>
          </div>
        )}
        ...
        {!loading && !error && scuList.length > 0 && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 animate-fade-in">
            {scuList.map((scu) => (
              <ScuCard key={scu.id} scu={scu} highlight={updatedScuIds.has(scu.id)} />
            ))}
          </div>
        )}
      </div>
      ...
      {showCreate && (
        <CreateScuModal onClose={() => setShowCreate(false)} onCreated={fetchScus} />
      )}
    </AppShell>
  )
}